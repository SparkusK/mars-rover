...
RSpec.describe InstructionsParser do
  context 'when it parses a string' do
    ...
    it 'parses the string into relevant objects', :aggregate_failures do
      outcome = described_class.new(input_string: input_string)
      ...
      expected_coordinator        = MarsRover::RoversCoordinator.new(rovers: expected_rovers, map: expected_map)

      expect(outcome.coordinator.rovers.first.position).to  eq(expected_rovers.first.position)
      expect(outcome.coordinator.rovers.first.bearing).to   eq(expected_rovers.first.bearing)
      expect(outcome.coordinator.rovers.last.position).to   eq(expected_rovers.last.position)
      expect(outcome.coordinator.rovers.last.bearing).to    eq(expected_rovers.last.bearing)

      expect(outcome.coordinator.map.width).to  eq(expected_map.width)
      expect(outcome.coordinator.map.length).to eq(expected_map.length)
